...
from scripts.config import HYPEROPT_EXPORT_DIR

# Enable unsafe deserialization for Keras models
keras.config.enable_unsafe_deserialization()

# Paths updated to exported hyperparameter optimisation directory
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_DIR = os.path.join(HYPEROPT_EXPORT_DIR)
...
async def lifespan(app: FastAPI):
    ...
    # Load hyperparameters and model weights from exported HPO directory
...
@app.post(
    "/predict/",
    responses={200: {"content": {"image/png": {}}, "description": "Predicted normal map as PNG image"}},
)
